const url = 'https://raw.githubusercontent.com/AbraaoDdm/json/refs/heads/main/preferencias.json';

const ctx = document.getElementById('grafico').getContext('2d');

let rotulosX = ["Minecraft", "CS", "FIFA22", "SonsofTheForest", "Fortnite", "Skyrim", "DragonBall"];
...
function atualizarGrafico() {
    fetch(url)
        ...
        .then(resp => {
            ...
            valores[1] = resp.CS;
            valores[2] = resp.FIFA22;
            valores[3] = resp.SonsofTheForest;
            valores[4] = resp.Fortnite;
            valores[5] = resp.Skyrim;
            valores[6] = resp.DragonBall;
...
function exibirFraseInformativa(url) {
    ...
    <p>Tais como <strong>Minecraft</strong>, <strong>Fortnite</strong> e <strong>DragonBall</strong> são os jogos que mais despertam meu interesse com um total de <span>${valores[0]}</span>, <span>${valores[4]}</span> e <span>${valores[6]}</span> pontos de interesse respectivamente, pois oferecem a oportunidade de mergulhar nos aspectos físicos e históricos de um fato..</p>
    <p>Além disso, <strong>FIFA22</strong> com <span>${valores[2]}</span> pontos, também é um dos jogos de destaque. Por outro lado, matérias como <strong>SonsofTheForest</strong> com <span>${valores[3]}</span> pontos, <strong>CS</strong> com <span>${valores[1]}</span> pontos e especialmente <strong>Skyrim</strong> com <span>${valores[5]}</span> pontos despertam menos meu interesse.</p>
    `;
}
